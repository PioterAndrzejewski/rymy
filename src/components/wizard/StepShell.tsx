...
export function Section({ title, hint, aside, children, muted = false }: SectionProps) {
  return (
    <Paper withBorder p="md" radius="md" style={{ opacity: muted ? 0.5 : 1, transition: 'opacity 200ms ease' }}>
      <Group justify="space-between" align="start" mb={hint ? 4 : 'sm'} wrap="nowrap">
    ...
    <Group justify="space-between" mt="xs">
      ...
      <Group gap="sm">
        {nextDisabled && blockedReason && (
          <Text size="xs" c="dimmed" ta="right" maw={280}>{blockedReason}</Text>
        )}
        <Button
          size="md"
          color="brand"
