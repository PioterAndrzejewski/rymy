...
import {
  Badge, Box, Button, Group, SimpleGrid, Stack, Switch, Text, UnstyledButton, Paper,
} from '@mantine/core';
import { useNavigate } from 'react-router-dom';
import {
  IconArrowLeft, IconChartBar, IconDice5, IconInfinity, IconListCheck, IconMetronome,
  IconRoute, IconStopwatch, IconTargetArrow,
...
import { RHYME_ENDINGS, corePool, rhymeCount } from '@/wordbank/pl/rhymes';
...
import { allReports, endingReport, pickPlanEnding, planReason, totals } from './family/review';
import {
  DURATIONS, QUOTA_CHOICES, WORD_SECONDS_CHOICES, defaultFamilyConfig, fmtDuration,
  isAutoEnding, minSecondsFor, rhymeWord, sessionModeLabel, type FamilyConfig,
} from './family/config';
...
export function RhymeFamily() {
  ...
  const sample = useMemo(
    () => (auto ? '' : corePool(config.ending).slice(0, 3).join(', ')),
    [auto, config.ending],
  );
  ...
  const planPick = useMemo(
    () => (sum.rounds === 0 ? null : endingReport(pickPlanEnding(progress), progress)),
    [progress, sum.rounds],
  );

  const endingHint = config.ending === 'plan'
    ? (planPick ? `program · np. -${planPick.ending}` : 'program')
  ...
  if (running) {
    return (
      <RhymeRun
        config={config}
        onExit={() => { setRunning(false); setProgress(loadProgress()); }}
      />
    );
  }
    ...
    return (
      <Stack gap="lg" my="md">
        <Group justify="space-between" align="end" wrap="wrap" gap="xs">
          <div>
            <Text style={{ fontSize: 'clamp(22px, 6.5vw, 26px)', fontWeight: 800, letterSpacing: '-0.02em' }}>Mój bank rymów</Text>
          ...
          <Button
            ...
            onClick={() => { setProgress(loadProgress()); setViewingProgress(false); }}
          >
            Do ćwiczenia
          </Button>
        </Group>
        <ProgressPanel
          onPractice={(e) => { patch({ ending: e }); setViewingProgress(false); setStep(1); }}
        />
      </Stack>
    );
  }
  ...
  return (
    <Stack gap="lg" my="md">
      ...
      {step === 0 && (
        <StepShell title="Wybierz końcówkę" description="Każda rodzina ma własny słownik rymów — po rundzie pokażemy, co ci umknęło, i wrócimy z tym później.">
          <Stack gap="md">
              ...
              <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="sm" mb="md">
                ...
                >
                  <Text size="xs" c="dimmed">
                    {planPick
                      ? <>Teraz padłoby na <b>-{planPick.ending}</b> — {planReason(planPick)}.</>
                ...
                <>
                      ...
                      return (
                        <UnstyledButton key={e} onClick={() => patch({ ending: e })}>
                          <Badge
                            ...
                            color={active ? 'brand' : r && r.rounds > 0 ? 'accent' : 'gray'}
                            style={{ cursor: 'pointer' }}
                            rightSection={
                              <Text span size="10px" c={active ? undefined : 'dimmed'}>
                                {r && r.rounds > 0 ? `${r.known}/${r.coreSize}` : rhymeCount(e)}
                              </Text>
                            }
                          >
                            -{e}
                  ...
                  <Text size="xs" c="dimmed" mt="sm">
                    ...
                    {(() => {
                      ...
                      if (!r || r.rounds === 0) return ' Tej jeszcze nie ćwiczyłeś.';
      ...
      {step === 1 && (
        <StepShell title="Jak lecimy" description="Jedno słowo przez całą rundę, czy kolejka słów jedno po drugim.">
          <Stack gap="md">
            ...
            {speechSupported && (
              <Section title="Mikrofon" hint="Rymowanie na głos jest bliższe temu, co robisz na scenie.">
                <Switch
                  size="md" color="brand"
                ...
                {config.voice && (
                  <Text size="xs" c="dimmed" mt="xs">
                    Przy włączonym mikrofonie metronom milczy — klik wchodziłby prosto w nasłuch.
                    {config.bpm > 0 && ' Załóż słuchawki, jeśli chcesz mieć jedno i drugie.'}
                    {' '}Na Androidzie rozpoznawanie mowy idzie przez serwery Google; reszta aplikacji
        ...
        <StepShell title="Wszystko gotowe" description="Zegar rusza od razu po kliknięciu.">
          <ReadyPanel
            ...
            items={[
              {
                label: 'Końcówka',
                value: config.ending === 'plan' ? 'program 🧭' : config.ending === 'random' ? 'losowa 🎲' : `-${config.ending}`,
              },
              ...
              { label: 'Czas', value: fmtDuration(config.seconds) },
              {
                ...
                value: config.bpm === 0
                  ? 'wyłączony'
                  : config.voice ? `${config.bpm} BPM (cichy)` : `${config.bpm} BPM`,
              },
              ...(speechSupported ? [{ label: 'Mikrofon', value: config.voice ? 'włączony 🎤' : 'wyłączony' }] : []),
            ]}
            note="Wpisuj rymy i zatwierdzaj Enterem. Słowo, które się nie rymuje, odbije się od pola — do banku trafia tylko to, co naprawdę pasuje."
          >
